...
import config
import src.kis_api as kis_api
import src.stock_master as stock_master
from src.position_manager import PositionManager
from web.routes import script as script_routes
from web.routes import nl_order as nl_order_routes

app = Flask(__name__)
app.register_blueprint(script_routes.bp)
app.register_blueprint(nl_order_routes.bp)
...
def get_stock(stock_code):
    ...
    try:
        ...
        res = kis_api._kis_request("get", f"{config.BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-price", headers=headers, params=params)
        out = res.json().get("output", {})
        price = int(out.get("stck_prpr", 0))
        prev = int(out.get("stck_sdpr", 0))
        change = price - prev
        # 종목별 내 계좌 정보를 위해 잔고 직접 조회
        try:
            holdings, deposit, avg_costs = kis_api.get_balance()
        except Exception as e:
            holdings, deposit, avg_costs = {}, 0, {}
...
def get_watchlist():
    try:
        import json as js
        wf = str(BASE_DIR / "watchlist.json")
...
def get_stock_news(stock_name):
    try:
        ...
        # 연예/스포츠/정치 등 무관 도메인 차단
        EXCLUDED_DOMAINS = {
            "tenasia.co.kr", "joynews24.com", "topstarnews.net", "sportsworldi.com",
            "chosun.com/sports", "donga.com/sports", "joins.com/sports", "yna.co.kr/sports",
            "yna.co.kr/entertainment", "sports.chosun.com", "sports.donga.com", "sports.joins.com",
            "isplus.com", "xportsnews.com", "spotvnews.co.kr", "mydaily.co.kr",
            "osen.co.kr", "news1.kr/articles/?418", "news1.kr/articles/?419", "news1.kr/articles/?420",
            "starnewskorea.com", "hankyung.com/entertainment", "mk.co.kr/star", "sedaily.com/Star",
            "dispatch.co.kr", "wikitree.co.kr", "insight.co.kr", "allkpop.com", "soompi.com",
        }
        EXCLUDED_KEYWORDS = [
            "연예", "가수", "배우", "영화", "드라마", "예능", "공개열애", "결별", "결혼", "이혼",
            "파파라치", "스포츠", "축구", "야구", "배구", "골프", "올림픽", "월드컵", "경기",
        ]
        ...
        def _is_relevant(item: dict) -> bool:
            ...
            # 종목명이 제목/설명에 직접 등장해야 관련 뉴스로 판단
            if keyword not in text:
                return False
            # 연예/스포츠 등 무관 키워드가 다수 등장하면 제외
        ...
        # 네이버 API display 최대 100까지 허용. 충분한 후보를 받아 필터링 후 25개 구성
        res = req.get(
            f"https://openapi.naver.com/v1/search/news.json?query={query}&display=100&sort=date",
            headers=headers
        )
        ...
        page_news = news[start:start + per_page]
        ...
        return jsonify({"error": str(e), "news": [], "page": 1, "total": 0, "total_pages": 1}), 200
    ...
    try:
        ...
        for code, name, key in [("0001","KOSPI","kospi"),("1001","KOSDAQ","kosdaq")]:
            try:
                ...
                res = kis_api._kis_request("get", f"{config.BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-index-price", headers=headers, params=params)
                out = res.json().get("output", {})
                result[key] = {
                    "name": name,
...
def run_screener():
    try:
        ...
        import sqlite3, os
        db_path = str(BASE_DIR / "stock_master.db")
        ...
        for code, name, market in stocks:
            ...
            try:
                ...
                params = {"FID_COND_MRKT_DIV_CODE": "J", "FID_INPUT_ISCD": code}
                res = kis_api._kis_request("get", f"{config.BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-price",
                    headers=headers, params=params)
                out = res.json().get("output", {})
                ...
                if rsi_min > 0 or rsi_max < 100:
                    try:
                        ...
                        r2 = kis_api._kis_request("get", f"{config.BASE_URL}/uapi/domestic-stock/v1/quotations/inquire-daily-itemchartprice",
                            headers=h2, params=cp)
...
# ===== 매매일지 =====
TRADES_FILE = str(BASE_DIR / "trades.json")
...
def get_auto_presets():
    try:
        pf = str(BASE_DIR / "auto_presets.json")
...
if __name__ == "__main__":
    ...
    # 백그라운드(nohup) 실행 시 reloader가 tty를 필요로 해서 termios 오류가
    # 발생할 수 있으므로, FLASK_DEBUG=1 환경변수가 있을 때만 debug 모드를 켭니다.
